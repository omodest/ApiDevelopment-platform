...
import api.development.platform.constant.FileConstant;
...
public class FileController {
    ...
    public BaseResponse<String> uploadFile(@RequestPart("file") MultipartFile multipartFile,
        ...
        try {
            // 上传文件
            file = File.createTempFile(filepath, null); // 创建个临时文件
            multipartFile.transferTo(file); // 将上传的文件写到临时文件中
            cosManager.putObject(filepath, file); // 将临时文件存到对象存储器中
            // 返回可访问地址
